...
string Creature::obtenirTypeCreature() const
{
	...
	if (energie_ >= pouvoir.obtenirEnergieNecessaire() && pouvoirEstDansVector)
	{
		if (creature.obtenirPointDeVie() >= 0) {
			...
			if (tentative != 3) {
				...
				cout << "Attaque " << pouvoir.obtenirNom() << " a echoue" << endl;
			}
		}
		else
...
ostream& operator<<(ostream& os, const Creature& creature) // À MODIFIER (si nécessaire...)
{
